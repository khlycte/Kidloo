local imageUrl = "https://photos.app.goo.gl/N3N2Tmt1E6VQgF467"
...
local function getCustomImage(url, fileName)
    if writefile and getgenv and getgenv().getcustomasset then
        if not isfile(fileName) then
            local success, response = pcall(function() return game:HttpGet(url) end)
            if success then writefile(fileName, response) end
        end
        if isfile(fileName) then return getgenv().getcustomasset(fileName) end
    end
    return "rbxassetid://138379435165992"
end

local imageAsset = getCustomImage(imageUrl, "kica_hub_icon.png")
...
SettingPage.CanvasSize = UDim2.new(0, 0, 0, 220)
...
createToggle(AIMPage, "KICA HUB", 40, Config.ShowFOV, function(v) Config.ShowFOV = v end)
createTextBox(AIMPage, "Đường kính FOV:", 75, Config.FOVRadius, function(v) Config.FOVRadius = v end)

createToggle(SettingPage, "Xoay Nhân Vật (Spin)", 5, Config.Spin, function(v) Config.Spin = v end)
createTextBox(SettingPage, "Tốc độ quay spin:", 42, Config.SpinSpeed, function(v) Config.SpinSpeed = v end)
createToggle(SettingPage, "góc nhìn thứ 3", 85, Config.ThirdPerson, function(v) Config.ThirdPerson = v end)
createToggle(SettingPage, "fix lag", 125, Config.FixLag, function(v)
    Config.FixLag = v
    if v then
        pcall(function()
            if setfpscap then
...
-- Hàm kiểm tra đồng đội nâng cao (Sửa lỗi nhận diện sai Team)
local function isRivalsEnemy(player)
    if player == LocalPlayer then return false end
    
    -- 1. Check Team mặc định của Roblox
    if LocalPlayer.Team and player.Team then
        return LocalPlayer.Team ~= player.Team
    end
    
    -- 2. Check Team/Role dạng Value Object (Dành cho game Rivals)
    local localTeamVal = LocalPlayer:FindFirstChild("Team") or LocalPlayer:FindFirstChild("TeamValue")
    local targetTeamVal = player:FindFirstChild("Team") or player:FindFirstChild("TeamValue")
    
    if localTeamVal and targetTeamVal then
        return localTeamVal.Value ~= targetTeamVal.Value
    end
    ...
    while task.wait(0.5) do
        for _, player in pairs(Players:GetPlayers()) do
            if player.Character then
                ...
                local hl = char:FindFirstChild("MultiTabHighlight")

                if Config.Chams and isRivalsEnemy(player) then
                    if not hl then
                        hl = Instance.new("Highlight")
                        hl.Name = "MultiTabHighlight"
...
-- Hàm tìm mục tiêu Aimbot (Bắt buộc phải có Highlight đỏ đang bật)
local function getValidTarget()
    ...
    for _, player in pairs(Players:GetPlayers()) do
        if isRivalsEnemy(player) and player.Character then
            ...
            -- Chỉ khóa aim khi đối phương ĐANG CÓ định vị đỏ
...
local spinAngle = 0
RunService.RenderStepped:Connect(function()
    if Config.AimAuto then
        local targetPos = getValidTarget()
        if targetPos then
    ...
    if Config.ThirdPerson then
        ...
        LocalPlayer.DevComputerCameraMode = Enum.DevComputerCameraMode.EnableClassic
        if LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("HumanoidRootPart") then
            if Camera.CameraSubject ~= LocalPlayer.Character:FindFirstChild("Humanoid") then
                Camera.CameraSubject = LocalPlayer.Character:FindFirstChild("Humanoid")
            end
            if (Camera.CFrame.Position - LocalPlayer.Character.HumanoidRootPart.Position).Magnitude < 2 then
